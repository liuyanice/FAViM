import os
import glob
import json
import torch
import random
import torch.nn as nn
import numpy as np
import torch.utils.data
from torchvision import transforms
import torch.utils.data as data
import torch.nn.functional as F
import cv2

import albumentations as A
from sklearn.model_selection import KFold


def norm01(x):
    return np.clip(x, 0, 255) / 255


seperable_indexes = json.load(open(r'/lab/ly/YanLiu/BUsegNew/BUsegNew/utils/TrainDataset/TG3K/data_split.json', 'r'))

seperable_indexes1 = json.load(open(r'/lab/ly/YanLiu/BUsegNew/BUsegNew/utils/TrainDataset/TG3K/data_split.json', 'r'))
# cross validation
class myDataset(data.Dataset):
    def __init__(self, fold, split, size=224, aug=False):#352 384 512
        super(myDataset, self).__init__()
        self.split = split

        # load images, label, point
        self.image_paths = []
        self.label_paths = []
        self.point_paths = []
        self.dist_paths = []

        indexes = os.listdir(
            r'/lab/ly/YanLiu/BUsegNew/BUsegNew/utils/TrainDataset/TG3K/Image/')#/raid/wjc/data/skin_lesion/isic2018_jpg_smooth/Image
        valid_indexes = [
            str(i) + '.png' for i in seperable_indexes1[str(fold)]
        ]
        train_indexes = list(filter(lambda x: x not in valid_indexes, indexes))
        print(len(indexes), len(train_indexes), len(valid_indexes))

        #valid_indexes = indexes[:260]
        #train_indexes = indexes[260:]
        print('Fold {}: train: {} valid: {}'.format(fold, len(train_indexes),
                                                    len(valid_indexes)))

        root_dir = r'/lab/ly/YanLiu/BUsegNew/BUsegNew/utils/TrainDataset/'

        if split == 'train':
            self.image_paths = [
                f'{root_dir}/TG3K/Image/{_id}' for _id in train_indexes
            ]
            self.label_paths = [
                f'{root_dir}/TG3K/Label/{_id}' for _id in train_indexes
            ]
        elif split == 'valid':
            self.image_paths = [
                f'{root_dir}/TG3K/Image/{_id}' for _id in valid_indexes
            ]
            self.label_paths = [
                f'{root_dir}/TG3K/Label/{_id}' for _id in valid_indexes
            ]
        else:
            self.image_paths = [
                f'{root_dir}/TG3K/Image/{_id}' for _id in valid_indexes
            ]
            self.label_paths = [
                f'{root_dir}/TG3K/Label/{_id}' for _id in valid_indexes
            ]

        print('Loaded {} frames'.format(len(self.image_paths)))
        self.num_samples = len(self.image_paths)
        self.aug = aug
        self.size = size

        p = 0.5
        self.transf = A.Compose([
            A.GaussNoise(p=p),
            A.RandomRotate90(p=p),
            A.HorizontalFlip(p=p),
            A.VerticalFlip(p=p),
            A.ShiftScaleRotate(p=p),
         #   A.ShiftScaleRotate(scale_limit=0.5, rotate_limit=0, shift_limit=0.1, p=p, border_mode=0),
           # A.RandomBrightnessContrast(p=0.2),
        #    A.GridDistortion(p=0.2),
          #  A.ElasticTransform(p=0.2)
            #             A.RandomBrightnessContrast(p=p),
        ])
    def get_boundary(self, mask, thicky=2):
        tmp = mask.astype('uint8')
        contour, _ = cv2.findContours(tmp, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        boundary = np.zeros_like(tmp)
        boundary = cv2.drawContours(boundary, contour, -1, 1, thicky)
        boundary = boundary *255.
        return boundary
    def __getitem__(self, index):
        # print(self.image_paths[index])
        image = cv2.imread(self.image_paths[index])
        image_data = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

        label_data = cv2.imread(self.label_paths[index], cv2.IMREAD_GRAYSCALE)
        label_data = np.array(
            cv2.resize(label_data, (self.size, self.size), cv2.INTER_NEAREST))
        boundary_data = self.get_boundary(label_data, thicky=2)
        point_data = cv2.Canny(label_data, 0, 255) / 255.0 > 0.5
      #  label_data[label_data > 100] = 255#
     #   label_data[label_data <= 100] = 0#
        label_data = label_data / 255. > 0.5
        boundary_data = boundary_data / 255. > 0.5
        image_data = np.array(
            cv2.resize(image_data, (self.size, self.size), cv2.INTER_LINEAR))

        # image_data = np.load(self.image_paths[index])
        # label_data = np.load(self.label_paths[index]) > 0.5
        # point_data = np.load(self.point_paths[index]) > 0.5
        # point_All_data = np.load(self.point_All_paths[index]) > 0.5  #

        #         label_data = np.expand_dims(label_data,-1)
        #         point_data = np.expand_dims(point_data,-1)
        if self.aug and self.split == 'train':
            mask = np.concatenate([
                label_data[..., np.newaxis].astype('uint8'),
                boundary_data[..., np.newaxis].astype('uint8'),
                point_data[..., np.newaxis]
            ],
                                  axis=-1)
            #             print(mask.shape)
            tsf = self.transf(image=image_data.astype('uint8'), mask=mask)
            image_data, mask_aug = tsf['image'], tsf['mask']
            label_data = mask_aug[:, :, 0]
            boundary_data = mask_aug[:, :, 1]
            point_data = mask_aug[:, :, 2]

        image_data = norm01(image_data)
        label_data = np.expand_dims(label_data, 0)
        boundary_data = np.expand_dims(boundary_data, 0)
        point_data = np.expand_dims(point_data, 0)
        # point_All_data = np.expand_dims(point_All_data, 0)  #

        image_data = torch.from_numpy(image_data).float()
        label_data = torch.from_numpy(label_data).float()
        boundary_data = torch.from_numpy(boundary_data).float()
        point_data = torch.from_numpy(point_data).float()
        # point_All_data = torch.from_numpy(point_All_data).float()  #

        image_data = image_data.permute(2, 0, 1)
        return {
            'image_path': self.image_paths[index],
            'label_path': self.label_paths[index],
            # 'point_path': self.point_paths[index],
            'image': image_data,
            'label': label_data,
            'boundary': boundary_data,
            'point': point_data,
            'point_All': label_data
        }

    def __len__(self):
        return self.num_samples


if __name__ == '__main__':
    from tqdm import tqdm
    dataset = myDataset(fold='0', split='train', aug=True)

    train_loader = torch.utils.data.DataLoader(dataset,
                                               batch_size=4,
                                               shuffle=False,
                                               num_workers=2,
                                               pin_memory=True,
                                               drop_last=True)
    import matplotlib.pyplot as plt
    for d in dataset:
        print(d['image'].shape, d['image'].max())
        print(d['point'].shape, d['point'].max())
        image = d['image'].permute(1, 2, 0).cpu()
        point = d['point'][0].cpu()
        plt.imshow(point)
        plt.show()
        break
