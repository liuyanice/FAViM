...
class in_scale_transformer(nn.Module):
    def __init__(self,
        ...
        self.query_embed = nn.Embedding(num_queries, d_model)# 1   128   d_model 是隐层的通道数
        self.pos_embed = PositionEmbeddingLearned(d_model // 2)# d_model=128   定义位置嵌入
        self.num_queries = num_queries
        #要修改的地方BoundaryAwareTransformer
    ...
    def forward(self, x):
      #  print('++++++++++++++++++++',x.shape)
      #  print('self.query_embed',self.query_embed.weight)
        pos_embed = self.pos_embed(x).to(x.dtype)# d_model=128   调用位置嵌入,生成位置嵌入
        latent_tensor, features_encoded = self.transformer(
            x, None, self.query_embed.weight, pos_embed)

        return latent_tensor, features_encoded
