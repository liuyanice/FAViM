...
from .multi_mamba import MultiScan, MultiScanVSSM
...
class SS2D(nn.Module):
    def __init__(
            ...
            simple_init=False,
            directions=None,
            deformable=True,  # 新增可变形扫描参数
            freq_aware=True,  # 新增频域感知参数
          #  win_size =8,
            **kwargs,
    ):
      ...
      #  self.multi_scan = MultiScanVSSM(d_expand, choices=directions)
        self.multi_scan = MultiScanVSSM(d_expand, choices=directions, deformable=deformable, freq_aware=freq_aware)
        if simple_init:
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.op = SS2D(
            ...
            deformable=deformable,  # 传递参数
            freq_aware=freq_aware,  # 传递参数
           # win_size =win_size,
        )
...
def CCViM(*args,in_chans=3,num_classes=1,dims=[96, 192, 384, 768],
                     ...
                     d_state=16, drop_path_rate=0.2, deformable=True, freq_aware=True, **kwargs):
    directions = [
        ['h', 'v', 'v_flip', 'w7_flip'],
        ['h', 'h_flip', 'v', 'w7'],
        ['h_flip', 'v', 'v_flip', 'w7'],
    ...
    return VSSM(in_chans=in_chans,num_classes=num_classes,dims=dims, depths=depths,dims_decoder=dims_decoder,
                ...
                directions=directions, decoder_directions=decoder_directions, deformable=deformable, freq_aware=freq_aware)
